...
> **Not:** Yarışma veri setleri (`data/`) `.gitignore` ile hariç tutulur. Eğitilmiş **Youden modeli** `models/<PANEL>/` altında depoya dahildir (`model_meta.json` + `stacking_ensemble.pkl` + `catboost_model.cbm`); `predict.py` bunları doğrudan kullanır, yeniden eğitim gerekmez.
...
Eğitim metrikleri tek başına aldatıcıdır (ilk model eğitimde daha yüksek F1 ve MCC verir); ama puanlanan test koşulunda dengeli eşik kazanır. Youden-J prevalanstan bağımsız olduğundan düşük-prevanslı gerçek test dağılımında da geçerlidir.
